...
parser.add_argument('--split', type=str, default='/cbica/home/shahroha/projects/AF-DIT/atlas/atlas.csv')
...
from utils import Protein, from_pdb_string
...
def main():
    ...
    do_job(args.atlas_dir)

def do_job(name):
    traj = mdtraj.load(f'{args.atlas_dir}/{name}_prod_R1_fit.xtc', top=f'{args.atlas_dir}/{name}.pdb') \
        + mdtraj.load(f'{args.atlas_dir}/{name}_prod_R2_fit.xtc', top=f'{args.atlas_dir}/{name}.pdb') \
        + mdtraj.load(f'{args.atlas_dir}/{name}_prod_R3_fit.xtc', top=f'{args.atlas_dir}/{name}.pdb')
    ref = mdtraj.load(f'{args.atlas_dir}/{name}.pdb')
    traj = ref + traj
